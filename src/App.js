...
const App = () => {
  ...
  const checkForRabbit = () => {
    ...
    return acceptableValues.includes(
      CARD_VALUES[playerCard.value] - CARD_VALUES[dealerCard.value]
    );
  };
  ...
  const checkForKhorosho = () => {
    ...
    return (
      acceptableValues.includes(playerCard.value) &&
      playerCard.suit === dealerCard.suit
    );
  };
